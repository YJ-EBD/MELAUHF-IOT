    ...
    def firmware_manage_atmega_page(request: Request):
        ...
        return templates.TemplateResponse(
            ...
            _base_context(
                ...
                rules_items=[
                    "1. 최초 1회는 ISP로 MELAUHF_ATmega128_boot_app_merged_uart1_250000.hex 와 퓨즈(LFUSE=0xBF, HFUSE=0xC2, EFUSE=0xFF)를 기록한 뒤 이 페이지를 사용합니다.",
                    "2. 이 페이지에는 ATmega용 Intel HEX 펌웨어만 업로드합니다.",
                    "3. 권장 업로드 파일은 hi-aba.hex 이며 build_id 는 uart1-fast-250k 형식으로 관리하는 것을 권장합니다.",
                    "4. 릴리스를 배정하면 ESP32-C5가 같은 장비의 ATmega128A를 UART1 fast bootloader 경로로 OTA 업데이트합니다.",
                    "5. 즉시 업데이트 버튼을 누르면 장비에 CHECK_OTA 명령을 보내 바로 검사와 적용을 시도합니다.",
                    "6. 업데이트 중에는 장비 운전을 멈춘 상태로 두는 것을 권장합니다.",
                ],
            ),
        )
