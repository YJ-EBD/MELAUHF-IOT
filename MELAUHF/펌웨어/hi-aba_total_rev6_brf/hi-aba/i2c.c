...
uint8_t adc_init()//return 0:success
{
	
	uint8_t buf[3];
	buf[0]=0x01;
	buf[1]=0x82;//ref=4.096V
	buf[2]=0x03;
	return twi_writeTo(0x48,buf,3,1,1);	
}
uint8_t adc_read(uint8_t *buf)//return 0: success
{	
	uint8_t regadd=ADS1115_REG_CONVERSION;
	//twi_writeTo(uint8_t address, uint8_t* data, uint8_t length, uint8_t wait, uint8_t sendStop)
	if(twi_writeTo(ADS1115_ADDR_GND,&regadd,1,1,1)==0)
	{
		if(twi_readFrom(ADS1115_ADDR_GND, buf, 2, 1)==2)
		{
			return 0;
		}		
	}
...
uint8_t gpio_out(uint8_t d)
{
	...
	buf[0]=0x01;
	buf[1]=d;
	return twi_writeTo(0x41,buf,2,1,1);	
}
...
uint8_t gpio_init()
{
	if(gpio_config(0xf8)!=0)
		return 1;
	if(gpio_out(0x00)!=0)
		return 1;
	return 0;
}
uint8_t gpio_read(uint8_t *d)
{
	uint8_t regadd=0;
	//twi_writeTo(uint8_t address, uint8_t* data, uint8_t length, uint8_t wait, uint8_t sendStop)
	if(twi_writeTo(0x41,&regadd,1,1,1)==0)
	{
		if(twi_readFrom(0x41, d, 1, 1)==1)
		{
			return 0;
		}
	}
