...
static const uint16_t Crc16_Table[CRC_TAB_SIZE] PROGMEM = {
...
uint16_t Generate_CRC (uint8_t *buf_ptr, int len)
{
	...
	{
		crc_16 = pgm_read_word(&Crc16_Table[(crc_16 >> 8) ^ *buf_ptr]) ^ (crc_16 << 8);
	}
	if (len != 0)
	{
		data = ((uint16_t) (*buf_ptr)) << (8);
		while (len-- != 0)
		{
			if ( ((crc_16 ^ data) & ((uint16_t) 1 << 15)) != 0 )
			{
				crc_16 <<= 1;
